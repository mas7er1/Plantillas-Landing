...
class MCAPI {
     ...
     * @param array $options a hash of the standard options for this campaign :
            ...
            boolean ecomm360 optional If set, our <a href="http://www.mailchimp.com/blog/ecommerce-tracking-plugin/"  >Ecommerce360 tracking</a> will be enabled for links in the campaign
    ...
    * @param array $type_opts optional - 
            For RSS Campaigns this, array should contain:
                ...
                array days optional used for "daily" schedules only, an array of the <a href="http://en.wikipedia.org/wiki/ISO-8601#Week_dates"  >ISO-8601 weekday numbers</a> to send on<div style="padding-left:15px"><table>
    ...
    */
    ...
    /**
     * Get the URL to a customized <a href="http://eepurl.com/gKmL"  >VIP Report</a> for the specified campaign and optionally send an email to someone with links to it. Note subsequent calls will overwrite anything already set for the same campign (eg, the password)
     *
     * @section Campaign  Related
     *
     ...
     * <a href="http://connect.mailchimp.com/category/ecommerce"  >provided by us or by 3rd part system developers</a>.
     * @section Campaign  Related
     *
     ...
     * @return array an array with keys listed in Returned Fields below
                    ...
                    array data the lists which matched the provided filters, including the following for 
                         ...
                         string visibility Whether this list is Public (pub) or Private (prv). Used internally for projects like <a href="http://blog.mailchimp.com/introducing-wavelength/"  >Wavelength</a>
     ...
     * @param array $options optional Various options for this merge var. <em>note:</em> for historical purposes this can also take a "boolean"
                    ...
                    string defaultcountry optional the <a href="http://www.iso.org/iso/english_country_names_and_code_elements"  >ISO 3166 2 digit character code</a> for the default country. Defaults to "US". Anything unrecognized will be converted to the default.
     ...
     * @param array $merge_vars optional merges for the email (FNAME, LNAME, etc.) (see examples below for handling "blank" arrays). Note that a merge field can only hold up to 255 bytes. Also, there are a few "special" keys:
                        ...
                        string date use YYYY-MM-DD to be safe. Generally, though, anything strtotime() understands we'll understand - <a href="http://us2.php.net/strtotime"  >http://us2.php.net/strtotime</a>
     ...
     *
     * @param string $id the list id to connect to. Get by calling lists()
     * @param string $status the status to get members for - one of(subscribed, unsubscribed, <a   href="http://eepurl.com/gWOO">cleaned</a>, updated), defaults to subscribed
     ...
     * @return array array of list members with their info in an array (see Returned Fields for details)
                ...
                array data an array of arrays where each one has member info:
                    ...
                    int member_rating the rating of the subscriber. This will be 1 - 5 as described <a href="http://eepurl.com/f-2P"  >here</a>
                    ...
                    string language if set/detected, a language code from <a href="http://kb.mailchimp.com/article/can-i-see-what-languages-my-subscribers-use#code"  >here</a>
                    bool is_gmonkey Whether the member is a <a href="http://mailchimp.com/features/golden-monkeys/"  >Golden Monkey</a> or not.
    ...
    /**
     * Retrieve the clients that the list's subscribers have been tagged as being used based on user agents seen. Made possible by <a href="http://user-agent-string.info"  >user-agent-string.info</a>
     *
     * @section List Related
     *
     * @param string $id the list id to connect to. Get by calling lists()
     ...
     * @param string $html a string specifying the entire template to be created. This is <strong>NOT</strong> campaign content. They are intended to utilize our <a href="http://www.mailchimp.com/resources/email-template-language/"  >template language</a>.
     ...
     * @param array  $values the values to updates - while both are optional, at least one should be provided. Both can be updated at the same time.
            ...
            string html optional a string specifying the entire template to be created. This is <strong>NOT</strong> campaign content. They are intended to utilize our <a href="http://www.mailchimp.com/resources/email-template-language/"  >template language</a>.
     ...
     * @section Helper
     *
     * @param string $query terms to search on, <a href="http://kb.mailchimp.com/article/i-cant-find-a-recipient-on-my-list"  >just like you do in the app</a>
    ...
    /**
     * Expire a Specific API Key. Note that if you expire all of your keys, just visit <a href="http://admin.mailchimp.com/account/api"  >your API dashboard</a>
